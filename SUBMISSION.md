...
**Prompt we picked:** #3 — turn messy documents into structured, queryable data  

**What we built:** Prism Core  

**Try it:** _(paste deployed URL)_ · locally http://localhost:3000 after `docker-compose up`  

For the design diagrams see [`HLD.md`](HLD.md). For “why this, not that” see [`decisions.md`](decisions.md). Papers we studied along the way: [`RESEARCH.md`](RESEARCH.md).

---

## How we read the prompt

The easy version of this assignment is: upload a PDF, call a big model, stash JSON, maybe add a search box.

We didn’t think that’s the interesting problem. The interesting problem is trust.
...
So we defined success as:

> Structured data you can defend — and a clear path when you can’t.

**In scope:** ingest, layout-aware extract, schema alignment with financial checks, sync into Postgres / Neo4j / Qdrant, chat over that data, HITL when we refuse to promote.

**Out of scope (on purpose):** full SSO, Kubernetes showmanship, every document type on earth, pixel-perfect UI polish. We’d rather go deep on statements, invoices, and bank docs than shallow on forty formats.
...
If the critic fails, we retry with the error fed back to the model a few times. If we still can’t fix it — or the failure looks permanent — it goes to HITL.

The longer version with diagrams is in [`HLD.md`](HLD.md).

---

## Where we went deep (the “above and beyond” bit)
...
4. **Accounting critics** — valid JSON can still be financially wrong; we fail closed and escalate.  
5. **CDC instead of multi-write** — one store down doesn’t corrupt the others.  
6. **Chat after sync** — SQL / graph / vectors with tenant filters injected by us, not the model.
...
We kept a running log in [`decisions.md`](decisions.md). Highlights:
...
We tried to be honest in that file about what’s fully live vs still thin (e.g. parts of near-dup handling).
...
On **100k+ documents:** the design supports a large corpus and horizontal ingest. Throughput is GPU/aligner-bound — you add replicas and partitions. Local compose is not a load test; don’t claim we soaked 100k on a laptop.
...
Then open the UI, upload something, watch the queue. For a strong demo, include one doc that *should* fail a critic and land in HITL — that moment shows we understood the problem.
